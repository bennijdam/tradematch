# TradeMatch Deployment Methodology Audit

## Scope
- Source of truth: repository tree and current Vercel configuration.
- Goals: confirm `apps/web` structure, auth page locations, Vercel root assumptions, and 404 risks.

## Directory Tree (apps/web, max depth 3)
```
apps/web/
|   .gitignore
|   404.html
...
## Auth Page Locations
- `auth-login.html` is in `apps/web/pages/auth-login.html`.
- `auth-forgot-password.html` is in `apps/web/pages/auth-forgot-password.html`.
- Both are inside the `pages` subfolder, not in the root of `apps/web`.

## Deployment Methodology (Current Vercel Setup)
- `vercel.json` uses `rewrites` that point to `/apps/web/...` paths, which implies the Vercel Root Directory is the repository root.
- Static HTML files are served from `apps/web`, with clean URLs mapped to specific HTML pages via rewrites and redirects.
- The configuration also includes a catch-all rewrite to `/apps/web/$1`, so any direct file path under `apps/web` should resolve if the root is the repo.

## 404 Risk Check: /forgot-password
- Current rewrite: `/forgot-password` -> `/apps/web/pages/auth-forgot-password.html`.
- That file exists under `apps/web/pages/`, so **no 404** if the Vercel Root Directory is the repo root.
- If the Vercel Root Directory is set to `apps/web`, the rewrite destination would effectively become `/apps/web/apps/web/pages/auth-forgot-password.html`, causing a 404.

## Notes
- There is also a fallback folder at `apps/web/forgot-password/index.html`, which can serve `/forgot-password` if rewrites are removed or adjusted for an `apps/web` root.
- The same root-directory assumption applies to most routes in `vercel.json`.
