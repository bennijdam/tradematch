const fs = require('fs');
const file = 'apps/web/help-centre.html';
let content = fs.readFileSync(file, 'utf8');

function h(hex) { return Buffer.from(hex, 'hex').toString('utf8'); }
...
for (const [search, replace] of replacements) {
  ...
  if (count > 0) {
    ...
    totalReplaced += count;
    console.log('Replaced ' + count + 'x: ' + Buffer.from(search).toString('hex').slice(0,12) + ' -> ' + replace);
  }
}
