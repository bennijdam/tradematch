(function () {
  ...
  const VIEW_BY_PATH = {
    ...
    '/user-dashboard/my-jobs': 'my-jobs',
    '/user-dashboard/quotes': 'quotes',
    '/user-dashboard/messages': 'messages',
    '/user-dashboard/saved-trades': 'saved-trades',
    '/user-dashboard/notifications': 'notifications',
    '/user-dashboard/billing': 'billing',
  ...
  function currentView() {
    ...
    return VIEW_BY_PATH[hashPath] || 'dashboard';
  }
  ...
  function switchMode() {
    ...
    const hashPath = (window.location.hash || '#/user-dashboard').split('?')[0];
    url.hash = `${hashPath}?mode=${encodeURIComponent(nextMode)}`;
    window.location.href = url.toString();
  }

  function injectToggle() {
    let button = document.getElementById('customer-mode-toggle');
    if (!button) {
      button = document.createElement('button');
      button.id = 'customer-mode-toggle';
      button.type = 'button';
  ...
  function renderNative() {
    let host = document.getElementById('customer-native-shell');
    if (!host) {
      host = document.createElement('div');
      host.id = 'customer-native-shell';
      host.style.position = 'fixed';
      host.style.inset = '0';
      host.style.zIndex = '2147483645';
      host.style.background = '#050709';
      host.innerHTML = '<iframe id="customerNativeFrame" title="TradeMatch Customer Native" style="width:100%;height:100%;border:0;display:block;background:#050709;"></iframe>';
      document.body.appendChild(host);
      document.documentElement.style.overflow = 'hidden';
      document.body.style.overflow = 'hidden';
    }

    const frame = document.getElementById('customerNativeFrame');
    if (!frame) return;

    const view = currentView();

    async function fetchLegacySnapshot(targetView) {
      const legacyPath = LEGACY_PATH_BY_VIEW[targetView] || LEGACY_PATH_BY_VIEW.dashboard;
      try {
        ...
        const fallbackRows = rows.length
          ? rows
          : Array.from(doc.querySelectorAll('.lead-card, .activity-item, .notification-item, .quote-card, .job-card'))
            .slice(0, 3)
            .map((node) => {
              const line1 = cleanText(node.querySelector('.lead-title, .activity-title, .notification-text, .quote-title, .job-title, h3')?.textContent || '');
              const line2 = cleanText(node.querySelector('.lead-time, .activity-description, .notification-time, .lead-meta, .quote-meta, .job-meta')?.textContent || '');
        ...
        return {
          ...
          metrics: safeMetrics,
          rows: fallbackRows,
          sideText: pickText(['.alert-content p', '.empty-state-desc', '.section-description', '.warning-content p'])
        };
      } catch (error) {
        return null;
      }
    ...
    const separator = NATIVE_SRC.includes('?') ? '&' : '?';
    frame.src = `${NATIVE_SRC}${separator}view=${encodeURIComponent(view)}&v=${encodeURIComponent(NATIVE_BUILD)}`;
    frame.addEventListener('load', () => {
      frame.contentWindow?.postMessage({
        type: 'customer-native-nav',
        view
      }, window.location.origin);
