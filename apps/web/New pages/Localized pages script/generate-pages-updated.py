...
import os, csv, json, random, hashlib, shutil, argparse
...
def normalize_meta_description(text, min_len=120, max_len=160):
    ...
    if len(text) < min_len:
        pad = " Free quotes, verified pros, and secure escrow protection."
        remaining = min_len - len(text)
        text = f"{text}{pad[:remaining]}" if remaining < len(pad) else f"{text}{pad}"
...
def gen_meta(service, location, pc, rc):
    ...
    text = _sub(t, service, location, PRO_COUNT=pc, REVIEW_COUNT=rc)
    return normalize_meta_description(text)
...
def fetch_pexels_trade_image(service, api_key, cache_dir):
    """Fetch one deterministic stock image per trade from Pexels and cache locally."""
    cache_dir.mkdir(parents=True, exist_ok=True)
    cached = cache_dir / f"{service['slug']}.jpg"
    if cached.exists() and cached.stat().st_size > 0:
        return cached

    query = parse.quote(service["name"])
    page = (_seed(service["slug"], service["slug"], "pexels-page") % 5) + 1
    search_url = (
        f"https://api.pexels.com/v1/search?query={query}&orientation=landscape"
        f"&size=large&per_page=30&page={page}"
    )

    req = request.Request(search_url, headers={"Authorization": api_key, "Accept": "application/json"})
    with request.urlopen(req, timeout=30) as res:
        payload = json.loads(res.read().decode("utf-8"))

    photos = payload.get("photos", [])
    if not photos:
        raise RuntimeError(f"No Pexels photos found for {service['name']}")

    photo = photos[_seed(service["slug"], service["slug"], "pexels-photo") % len(photos)]
    src = photo.get("src", {})
    img_url = src.get("landscape") or src.get("large2x") or src.get("large") or src.get("original")
    if not img_url:
        raise RuntimeError(f"No usable Pexels image URL for {service['name']}")

    img_req = request.Request(img_url, headers={"Authorization": api_key})
    with request.urlopen(img_req, timeout=45) as img_res:
        data = img_res.read()
...
def fetch_unsplash_trade_image(service, api_key, cache_dir):
    """Fetch one deterministic stock image per trade from Unsplash and cache locally."""
    cache_dir.mkdir(parents=True, exist_ok=True)
    cached = cache_dir / f"{service['slug']}.jpg"
    if cached.exists() and cached.stat().st_size > 0:
        return cached
    ...
    query = parse.quote(service["name"])
    search_url = f"https://api.unsplash.com/photos/random?query={query}&orientation=landscape"
    req = request.Request(
        search_url,
    ...
    if not img_url:
        raise RuntimeError(f"No usable Unsplash image URL for {service['name']}")
...
def build_page_hero_image(
    ...
    """Generate or resolve page hero image path to inject into HTML."""
    if image_strategy != "pexels-watermark" or hero_source is None:
        return "hero-background.webp"

    rel_asset = f"assets/hero/{service['slug']}/{location['slug']}.webp"
    abs_asset = output_dir / rel_asset
    abs_asset.parent.mkdir(parents=True, exist_ok=True)

    watermark_text = f"Verified in {location['name']} • {service['name']}"
    if not abs_asset.exists():
        ok = try_watermark(hero_source, abs_asset, watermark_text)
        if not ok:
            fallback = output_dir / "hero-background.webp"
            if fallback.exists():
                shutil.copy(fallback, abs_asset)

    if asset_base_url:
        base = asset_base_url.rstrip("/")
        return f"{base}/{rel_asset.replace(os.sep, '/')}"

    return f"../../{rel_asset.replace(os.sep, '/')}"
...
def parse_args():
    parser = argparse.ArgumentParser(description="TradeMatch localized SEO page generator")
    parser.add_argument("--yes", action="store_true", help="Skip interactive confirmation prompt")
    parser.add_argument("--dry-run", action="store_true", help="Run generation logic without writing files")
    parser.add_argument(
        "--services",
        type=str,
        default="",
        help="Comma-separated list of service slugs to generate",
    )
    parser.add_argument(
        "--locations-limit",
        type=int,
        default=0,
        help="Limit number of locations processed",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Cap total pages generated (useful for pilot runs, e.g. 100)",
    )
    parser.add_argument(
        "--image-strategy",
        type=str,
        default="local-hero",
        choices=["local-hero", "pexels-watermark"],
        help="Image strategy: local-hero (default) or pexels-watermark",
    )
    parser.add_argument(
        "--pexels-api-key",
        type=str,
        default="",
        help="Optional Pexels API key; if omitted uses PEXELS_API_KEY env var",
    )
    parser.add_argument(
        "--asset-base-url",
        type=str,
        default="",
        help="Optional CDN base URL for rendered image paths, e.g. https://assets.tradematch.uk",
    )
    parser.add_argument(
        "--stock-provider",
        type=str,
        default="auto",
        choices=["auto", "pexels", "unsplash"],
        help="Stock image provider selection when image strategy is pexels-watermark",
    )
    parser.add_argument(
        "--unsplash-api-key",
        type=str,
...
def filter_services(service_filter):
    if not service_filter:
        return SERVICES

    selected_slugs = [slug.strip() for slug in service_filter.split(",") if slug.strip()]
    if not selected_slugs:
        return SERVICES

    by_slug = {service["slug"]: service for service in SERVICES}
    invalid = [slug for slug in selected_slugs if slug not in by_slug]
    if invalid:
        raise ValueError(f"Unknown service slug(s): {', '.join(invalid)}")

    return [by_slug[slug] for slug in selected_slugs]
...
def main():
    ...
    try:
        selected_services = filter_services(args.services)
    except ValueError as exc:
        print(f"ERROR: {exc}")
        return

    locations = load_locations()
    if args.locations_limit and args.locations_limit > 0:
        locations = locations[: args.locations_limit]

    county_index = build_county_index(locations)
    total_pages = len(selected_services) * len(locations)
    target_pages = min(total_pages, args.max_pages) if args.max_pages and args.max_pages > 0 else total_pages

    print()
    print(f"Generation Plan:")
    print(f"  Services  : {len(selected_services)}")
    ...
    if hero_src.exists() and not args.dry_run:
    ...
    pexels_key = args.pexels_api_key.strip() or os.getenv("PEXELS_API_KEY", "").strip()
    unsplash_key = args.unsplash_api_key.strip() or os.getenv("UNSPLASH_ACCESS_KEY", "").strip()
    pexels_sources = {}
    if args.image_strategy == "pexels-watermark":
        has_any_provider_key = bool(pexels_key or unsplash_key)
        if not has_any_provider_key:
            print("WARNING: No stock API key found. Falling back to local-hero mode.")
            args.image_strategy = "local-hero"
        elif not args.dry_run:
            print("Preparing stock source images per trade...")
            cache_dir = OUTPUT_DIR / ".cache" / "pexels"
            for service in selected_services:
                try:
                    image_path = None
                    if args.stock_provider in ["auto", "pexels"] and pexels_key:
                        image_path = fetch_pexels_trade_image(service, pexels_key, cache_dir)
                    if image_path is None and args.stock_provider in ["auto", "unsplash"] and unsplash_key:
                        image_path = fetch_unsplash_trade_image(service, unsplash_key, cache_dir)

                    pexels_sources[service["slug"]] = image_path
                except (RuntimeError, error.URLError, TimeoutError, error.HTTPError) as exc:
                    print(f"  WARN {service['slug']}: {exc}; using local hero fallback")
                    pexels_sources[service["slug"]] = hero_src if hero_src.exists() else None
    ...
    for idx, service in enumerate(selected_services, 1):
        ...
        for location in locations:
            ...
            if args.image_strategy == "pexels-watermark":
                hero_source = pexels_sources.get(service["slug"], hero_src)

            hero_image_url = build_page_hero_image(
                service=service,
                location=location,
    ...
    if not args.dry_run:
        ...
        if generated == total_pages:
            sm_files = generate_sitemap(selected_services, locations)
        elif generated > 0:
            flat_urls = []
            for service in selected_services:
                for location in locations:
        ...
        with open(OUTPUT_DIR / "page-data.json", "w", encoding="utf-8") as f:
            json.dump(
                {
    ...
    print(f"Sitemaps: {len(sm_files) if not args.dry_run else 0}")
    print()
    print("Next steps:")
